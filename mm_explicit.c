...
//implicit(명시적) 할당기의 명시적 가용 리스트 구현
...
{   
    ...
    if ((heap_listp = mem_sbrk(6*WSIZE)) == (void *)-1) //heap_listp가 힙의 최댓값 이상을 요청한다면 fail, 16바이트를 호출한다.
        return -1;    
    ...
    free_listp = heap_listp + DSIZE;    //free_listp 초기화 PRED 포인터 앞쪽으로 옮긴다.
    
    
    /* extend_heap 함수를 호출해서 힙을 CHUNKSIZE 바이트로 확장하고 초기 가용 블록을 생성한다.
        여기 까지 할당기는 초기화를 완료하고, 어플리케이션으로 부터 할당과 반환 요청을 받을 준비를 완료하였다.*/
    if (extend_heap(CHUNKSIZE / WSIZE) == NULL) //WSIZE로 나눌지 DSIZE로 나눌지 정하자. 
        return -1;
...
void *mm_malloc(size_t size)
{
    ...
    if((bp = extend_heap(extendsize/WSIZE)) == NULL)    //칸의 개수 //여기도 WSIZE로 할지 DSIZE로 나눌지 정하자
        return NULL;
    // 확장하여 커진 힙 위치에 할당한다.
    place(bp,asize);
    return bp;    
...
static void *coalesce(void *bp)
{   
    size_t prev_alloc = GET_ALLOC(HDRP(PREV_BLKP(bp))); //푸터에서 헤더로 바꾼 이유?
    size_t next_alloc = GET_ALLOC(HDRP(NEXT_BLKP(bp)));
    size_t size = GET_SIZE(HDRP(bp));

    // 이미 가용리스트에 존재하던 블록은 연결하기 이전에 가용리스트에서 제거해준다.
    if (prev_alloc && !next_alloc) { //이전 블록 할당블록이고 다음블록이 가용 블록이면,
...
static void *find_fit(size_t asize)
{
    ...
    void *bp;
    bp = free_listp;
    //가용 리스트 내부의 유일한 할당 블록은 맨 뒤의 프롤로그 블록이므로 할당 블록을 만나면 for문을 종료
...
void putFreeBlock(void *bp) // 후입 선출 규칙을 위해 항상 새로생긴 가용블록을 맨 앞에 추가한다.
{
...
void removeBlock(void *bp) // 블록을 삭제하고 전 후 블록간의 관계를 조정해주는 함수
{
    //첫번째 블록을 없앨 때
    if(bp == free_listp){
